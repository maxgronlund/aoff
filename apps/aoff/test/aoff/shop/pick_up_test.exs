defmodule AOFF.Shop.PickUpTest do
  ...
  describe "pick_ups" do
    ...
    test "list_pick_ups/0 returns all pick_ups", %{date: date, user: user, order: order} do
      ...
      assert List.first(Shop.list_pick_ups("public", date.id)).id == pick_up.id
    end
    ...
    test "list_upcomming_pick_ups/0 returns all upcomming pick_ups for a given user", %{
      ...
      dates = Shop.list_upcomming_pick_ups("public", user.id, Date.utc_today())
    ...
    test "list_upcomming_pick_ups/0 returns false", %{date: date, user: user, order: order} do
      ...
      pick_ups = Shop.list_upcomming_pick_ups("public", user.id, next_date)
    ...
    test "get_pick_up!/1 returns the pick_up with given id", %{
      ...
      pick_up = create_pick_up(date, user, order)
      assert Shop.get_pick_up!("public", pick_up.id).id == pick_up.id
    end
    ...
    test "create_pick_up/1 with valid data creates a pick_up", %{
      ...
      assert {:ok, %PickUp{} = pick_up} = Shop.create_pick_up("public", attrs)
    ...
    test "create_pick_up/1 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} = Shop.create_pick_up("public", attrs)
    end
    ...
    test "update_pick_up/2 with invalid data returns error changeset", %{
      ...
      assert pick_up.id == Shop.get_pick_up!("public", pick_up.id).id
    end

    test "delete_pick_up/1 deletes the pick_up", %{date: date, user: user, order: order} do
      pick_up = create_pick_up(date, user, order)
      assert {:ok, %PickUp{}} = Shop.delete_pick_up(pick_up)
      assert_raise Ecto.NoResultsError, fn -> Shop.get_pick_up!("public", pick_up.id) end
    end
