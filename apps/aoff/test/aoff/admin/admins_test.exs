defmodule AOFF.Admin.AdminsTest do
  ...
  describe "users" do
    ...
    test "list_users/0 returns all users" do
      user = user_fixture()
      assert List.first(Admins.list_users()).id == user.id
    end

    test "list_shop_assistans/0 returns all shop_assistans" do
      user = user_fixture(%{"shop_assistant" => true})

      assert List.first(Admins.list_shop_assistans()).id == user.id
    end
    ...
    test "username/1 returns the username for a given shop_assistans" do
      ...
      assert Admins.username(user.id) == user.username
    end

    test "get_user!/1 returns the user with given id" do
      user = user_fixture()
      assert Admins.get_user!(user.id).id == user.id
    end
    ...
    test "update_user/2 with invalid data returns error changeset" do
      ...
      assert user.id == Admins.get_user!(user.id).id
    end
